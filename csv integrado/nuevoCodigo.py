...
# Load CSV
csv_path = 'csv integrado/Datos_fotos.csv'
df = pd.read_csv(csv_path)

# Image directory
image_dir = 'csv integrado/fotos de chicos 100 M 2025-20250903T165147Z-1-001/fotos de chicos 100 M 2025'

# Output directory
os.makedirs('csv integrado/dataUnificado', exist_ok=True)
...
for name, group in df.groupby('Nombre'):
    ...
    if face_data_list:
        ...
        face_filename = os.path.join('csv integrado/dataUnificado', f'faces_{name}.pkl')
        with open(face_filename, 'wb') as w:
            pickle.dump(face_data, w)

        name_filename = os.path.join('csv integrado/dataUnificado', f'names_{name}.pkl')
