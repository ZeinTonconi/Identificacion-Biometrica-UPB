...
while ret:
    ...
    if ret:
        ...
        # Display instruction
        cv2.putText(frame, "Presiona 'c' para capturar (ESC para salir)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        if key == 27:  # ESC to exit
            ...
            # Augmentation 1: Rotate +10 degrees (simulate head tilt right)
            center = (25, 25)
            M = cv2.getRotationMatrix2D(center, 10, 1.0)
            rot1 = cv2.warpAffine(resized, M, (50, 50))
            face_data.append(rot1)
            
            # Augmentation 2: Rotate -10 degrees (simulate head tilt left)
            M = cv2.getRotationMatrix2D(center, -10, 1.0)
            rot2 = cv2.warpAffine(resized, M, (50, 50))
            face_data.append(rot2)
            
            # Augmentation 3: Horizontal flip (simulate mirror image for facial symmetry)
            flipped = cv2.flip(resized, 1)
            face_data.append(flipped)
            
            # Augmentation 4: Increase brightness (handle varying lighting conditions)
            bright = cv2.convertScaleAbs(resized, alpha=1.0, beta=20)
            face_data.append(bright)
...
if captured:
    ...
    face_filename = os.path.join('data2', f'faces_{name}.pkl')
    with open(face_filename, 'wb') as w:
        pickle.dump(face_data, w)
    
    # Save name data with corresponding filename
    names = [name] * 5
    name_filename = os.path.join('data2', f'names_{name}.pkl')
